...
## 1.28 The preview finally shows what the stack will look like — 2026-08-19

T-4.6 closes Phase 2's algorithmic arc. Its first half was already done: T-4.4 writes each frame's
transform into `FrameRecord.transform`, and `SessionLogTest` has had a round-trip test for that
field since Phase 1C, written before anything could fill it.

The second half is the one worth recording. **The live preview has been aligning frames by
translation alone**, voted between consecutive frames by `StarOffset`. That is correct for a tripod
that only drifts and wrong for every alt-az mount, where the field also turns. Rotation was
therefore accumulating into the preview as a smear that grew through the session — worst at the
corners, invisible at the centre, and indistinguishable from soft focus to anyone looking at it on
a phone in the dark. Now that a measured transform exists per frame, the preview can show what the
stack will actually look like, which is the only reason to have a preview at all.

Measured on a synthetic session rotating 0.4° a frame over eight frames — under three degrees in
total, which an alt-az mount reaches in minutes near the zenith — stacking through the real
transform gives **tighter stars and more of them** than the translation-only path, because a
smeared star spreads its flux and drops below the detection threshold.

### The risk was coordinates, not geometry

Rotating a raster is well-trodden. What is not is that **three coordinate systems meet in one
expression**: preview pixels, the binned analysis plane, and the sensor coordinates the transform
was measured in. Get the composition wrong and nothing throws — the preview is simply a little
soft, which is also what bad focus, bad seeing and a perfectly good stack on a mediocre night all
look like.

So it is `PlaneMapping`, one documented place, tested against the transform it must reproduce
rather than against itself. Three specific traps have tests of their own:

- **A sensor translation must be divided by the bin factor.** Applied raw it moves the frame four
  times too far — still alignment, just bad alignment.
- **The `binOffset · (a + b − 1)` term is small and not zero.** A naive `tx / binFactor` drops it;
  under rotation it is a fraction of a pixel, which is exactly the size of error that survives
  every visual check and still smears a hundred-frame stack.
- **Rows must stay rows without rotation, and must not with it.** A transposed matrix breaks the
  first; a mapping that quietly ignored its cross terms would pass everything else while degrading
  to the very behaviour this task replaces.

### One thing deliberately not deleted

`StarOffset`'s translation-only path is kept as a fallback. It is close to unreachable now — an
accepted light has either a transform or is the reference — but registration **has never run under
...
## 1.38 On the phone at last — four findings, one of them architectural — 2026-09-02

Everything from §1.33 to §1.37 was built without a device. The phone was plugged in and the queue
run. Nothing in the arithmetic was wrong; almost everything about the *shape* of the work was.

### There is field data, and this document said there was not

`2026-08-23_0006`, sitting on the phone since 2026-08-23 and unmentioned here: **120 lights, 119
accepted** (one BUMPED), **18 darks**, 7.4 s subs at ISO 3200, **14.7 minutes of integration**, 118
transforms plus the reference, and HFR, star count and background populated on every accepted
frame. It has never been stacked.

§1.29's "every field frame was destroyed" was true on 2026-08-19 and has been stale since
2026-08-23. **T-5.7 is not blocked on a night.** It is blocked on the stack finishing.

### T-5.4's profile, and §1.33's estimate was low by five times

`--es diag combine`, on the reference phone:

    20 frames — sigma-clip 12613.2 ms/MP, mean  397.2 ms/MP (31.8x), 0.269% rejected
    50 frames — sigma-clip 13042.3 ms/MP, mean  516.8 ms/MP (25.2x), 0.006% rejected
   150 frames — sigma-clip 37433.2 ms/MP, mean 1500.9 ms/MP (24.9x), 0.001% rejected
   150 frames — a 12.6 MP master is 471.7 s of combining, on one core

**Nearly eight minutes of combining for a 150-frame master, single-threaded.** §1.33 guessed
"plausibly several times the warp pass's 25–29 s" and it is **sixteen times** it. The rejection
figures are consistent with §1.33 — the generator plants one intruder per affected pixel regardless
of frame count, so the falling rate is arithmetic rather than a weakening clip, and the 20-frame
number is the iteration nibbling §1.33 measured at 0.88%.

**§12.1's JNI question stays closed, and for the reason §1.33 predicted.** The loop is per-pixel
independent and the phone has eight cores; threading turns 7.9 minutes into one or two. JNI might
buy two or three times on top of that and costs a native library and a boundary. Threads first.

### The heap does not fit a stack, and the arithmetic was written down three times

The first real run died at tile one: `OutOfMemoryError, target footprint 268435456`. A 12.6 MP
master is 151 MB, the master dark 50 MB, the tiled sample buffer up to 96 MB — **~300 MB against a
256 MB growth limit**. Every one of those numbers is stated in a doc comment in this codebase
(§1.32, §1.34, §1.35) and **nobody added them up**. `largeHeap="true"` takes the limit to 512 MB on
this device and the run proceeds.

### The margin is both too small and ruinously expensive — and this is architectural

Two facts that only met on real data.

**Too small.** The session rotates **3.72°** over its 14.7 minutes, which is exactly the field
rotation an alt-az mount produces in that time. The largest row displacement anywhere in the frame
is **219.5 rows**. The margin is a fixed **160** (§1.32), so rows that a tile needs are not being
fetched — which is precisely the seam the margin exists to prevent, arriving because the constant
was sized for "the 2–3° a session reaches" and a real session reached more.

**Ruinously expensive.** `tileRowsFor` sizes the tile from the sample budget, and 114 frames with
T-5.5's weighting gives **8 rows**. Each tile therefore reads, calibrates, debayers *and warps* a
band of `8 + 2×160 = 328` rows to produce 8 — a **41× amplification** of every per-pixel cost
before the combine. Correcting the margin to 220 would make it **56×**.

Measured on the real session: **50 tiles of 384 in eight minutes, so about 61 minutes for one
stack**, of which roughly 16 minutes is warping pixels that are thrown away and 117 GB is read from
flash to produce a 151 MB master.

**The tiled design cannot be tuned out of this.** The margin is `frame width × sin(rotation)`, so it
grows with session length; the tile shrinks as frames are added. The two move in opposite
directions and both get worse exactly as a session gets better.

**The fix is to warp once rather than per tile.** Register each frame into reference coordinates a
single time, write it to an intermediate, then tile the *combine* over already-registered data —
where the margin is zero, because every frame is already in the same coordinate system. On this
session: one warp pass of 22 s, 8.6 GB of 16-bit intermediates against 44 GB free, and a combine
pass that reads them straight through. **About 7–8 minutes, dominated by the combine**, which is
then the thing threading attacks.

That is how desktop stackers are built, and it is the shape T-5.3 should have had. It was not
knowable without this measurement: the amplification is invisible at the 24-row fixture size the
JVM tests use, where the margin clips to the whole frame and every band is the whole frame — the
same blind spot §1.34 recorded, showing up a second time in a different disguise.

### What is now true

- **T-5.4's profile: done.** §12.1's exception stays shut; threads are the lever.
- **T-5.1's throughput stands** — the warp is fast; the loop calls it 41 times too often.
- **A stack has never completed on real data.** The pipeline runs, reads real DNGs, applies real
  darks and real transforms, and is too slow to finish in a sitting.
- **T-5.7 is one architectural fix away**, not one clear night away.
...
## 1.42 The picture, on the phone, with a slider — 2026-09-03

T-7.5 and T-7.6, and **walked on the device rather than reasoned about**: opened the session,
tapped through to the result, dragged the slider from 55% to 94% and watched the sky lift, saved,
and found the file in `Pictures/StarStacker`. First time in this project that a night's work can be
seen without a laptop.

### Two resolutions, which is the whole reason a slider is possible

A full render is 132 MB read and edited — fine once, hopeless while a finger is moving. So the
linear master is read back **decimated** to about 1024 pixels across, and every slider movement
re-renders that: 9 MB, imperceptible. Saving renders the full frame once, when the answer is
settled.

`LinearMaster.read` **decimates rather than averaging**, and that is deliberate: averaging would be
a better image and a worse preview, because it would quietly reduce the noise, so the stretch
measured on it would not be the stretch the full-size render needs and the slider would lie about
what it was setting.

**Every render starts from the linear master, never from the last render.** That is what makes it a
slider rather than a ratchet — drag it back and you get exactly what you had, because nothing
accumulated. FR-8.2's insistence on keeping the linear data is what buys this.

### The app can read its own output

`LinearMaster` could write a float TIFF and not open one, which was fine while the only consumer
was Siril. The reader is deliberately **strict** — it checks compression, channel count and
`SampleFormat` and refuses anything else — because there is exactly one producer and a tolerant
reader here would be inventing support for files that do not exist.

### Before/after is a press, not two images

Side by side at phone width gives two pictures too small to judge. Tapping the image shows the
linear data as it came off the stack; tapping again returns. The comparison happens in the same
pixels, which is the only way to see what the edit did to them.

### And the list was lying

The session row said `Captured` next to a session with a master in it, which was noticed on the
phone within a minute of the first stack finishing. The badge now reads **`Stacked`**, and an
unstacked one reads **`Stack now`** — which is the *action* the prototype specified and that
§6.5's note said would come back "when T-5.x turns it into one". It has.

**The thumbnail is still a `NO STACK` placeholder**, which is T-6.1 and was always waiting for
exactly this: there is now a stretched JPEG per session to put there.

### What was verified, and how

Not by reading the code. `uiautomator` to find the controls, `input tap` and `input swipe` to drive
them, `screencap` to see the result, and `content query` on `MediaStore` to confirm the row:

    _display_name=2026-08-23_0006.jpg, relative_path=Pictures/StarStacker/, _size=4663175

The slider readout tracked correctly — *55% · the sky sits at 0.23, colour ×1.33* became *94% · the
sky sits at 0.33, colour ×1.56* — and the image visibly brightened between the two screenshots.

### What is left

**The gallery copy is published from the service, not from `StackJob`.** A stack run from the
button publishes; one run from `--es diag stack` does not, deliberately — a diagnostic has no
business writing to someone's photo library.

**No 16-bit stretched TIFF.** FR-9.1 lists `stack_stretched.tif` beside the JPEG and only the JPEG
exists. The JPEG is what a person shares; the TIFF is what they would take somewhere else to
finish, and that is worth having once T-8.3's flats make the background worth finishing.
...
## 1.45 Moon mode, proposed — and why it is a mode rather than a setting — 2026-09-08

Session `2026-09-06_0118` caught a crescent moon over a harbour and **destroyed it**: 1 597 pixels
of the disc pinned at the sensor's white level of 1023, with the histogram showing a hard spike at
the top and almost nothing between 200 and 1000 ADU. No amount of stacking recovers a clipped
pixel. The frames are a fine landscape and a ruined moon.

This proposes the mode that would have prevented it.

### How far out the exposure was, exactly

The moon is not a faint object. It is a sunlit rock at the same distance from the sun as we are, so
its surface brightness is roughly that of a grey card in daylight — the "Looney 11" rule, `f/11` at
`1/ISO` seconds.

| | |
|---|---|
| camera 3 is `f/2.55`, gathering `(11/2.55)² = 18.6×` more light than `f/11` | **4.22 stops** |
| so a correct exposure at ISO 100 is | **0.54 ms** |
| at ISO 400, what the session used | **0.13 ms** |
| what the session actually shot | **2 474.6 ms** |
| overexposed by | **18 419× — 14.2 stops** |

Even crediting a thin crescent (~2 stops, because the low sun angle across the surface dims it) and
2° altitude (~3 stops of atmospheric extinction), it is still **nine stops over**. This is not a
tuning error. It is the deep-sky exposure engine being pointed at a daylit object, and no session
planner built around sky background and trailing limits will ever produce the right answer for it.
...
the mode **probes and measures**, targeting the brightest part of the disc at about **70%** of white
— below clipping, with margin for the seeing to flicker.

That loop already exists and has already proved itself: `FlatCheck`'s metering went 2 ms → 45.9 ms →
52% of full scale in two probes (§1.44). It was written for flats and it is the same problem.

### Lucky imaging, which the codebase is oddly well set up for

Lunar detail is limited by atmospheric seeing, and the answer is *many short frames, keep the
sharpest few*. **T-5.5's keep-best-N% cut already does exactly this** — it needs only a different
quality metric, because HFR and star count mean nothing on a lunar disc. Sharpness does: gradient
energy or Laplacian variance over the disc, which rewards a crisp terminator.

The same metric doubles as the **focus** signal, replacing T-2.4's HFR star sweep with contrast
maximisation on the moon itself.

### The tension worth stating before anyone starts

**Storage.** At 0.5 ms the sensor could deliver hundreds of frames in seconds, and lucky imaging
wants them — but this app writes a 25 MB DNG per frame, so 500 frames is 12.5 GB for an object
occupying 0.03% of the frame. Real lunar imaging uses cropped video for exactly this reason.

Three ways out, and the choice should be made deliberately: cap the frame count and accept less
luck; crop on write, which means a new on-disk shape; or keep full frames and treat the moon as a
short session. **The first is the honest v1** and the third is what a nightscape wants anyway.

### What gets simpler, for once

- **Darks are nearly free.** At 0.5 ms dark current is negligible and a dark is essentially a bias
  frame, so a handful suffices.
- **Flats barely matter.** A 75 px object in the middle of the frame sees almost none of the 4×
  vignetting that dominates a wide-field sky (§1.41).
...
Moon mode **needs non-star alignment** — a lunar frame has no star field to match — and that is
precisely what T-4.7's fallback is. They want the same primitive: align two frames without stars,
by phase correlation or centroid.

So the honest sequencing is **build the primitive once**. T-4.7 becomes its first consumer (rescue
a session star matching could not handle) and T-11.5 its second (align on a chosen object by
design). Doing moon mode does not make T-4.7 redundant; it makes it cheap.
...
- [~] **T-4.6** Transforms written into `session.json`; live preview stack upgraded to true aligned
  accumulation.
  **Built 2026-08-19**, 11 tests, §1.28. The first half landed with T-4.4 — `FrameRecord.transform`
  carries the six affine terms in sensor coordinates, and `SessionLogTest` had a round-trip test
  waiting for it since Phase 1C.
  **The second half is the substantial one.** The preview aligned by a *translation* voted between
  consecutive frames, so field rotation accumulated into it as a smear that grew with the session —
  worst at the corners, invisible at the centre, and indistinguishable from poor focus to anyone
  looking at it. It now samples through the measured affine transform, so the preview shows what
  the stack will actually look like, which is the entire point of having one.
  **The risk was coordinates, not geometry**: preview pixels, the binned plane and sensor
  coordinates all meet in one expression, and getting it wrong produces a preview that is merely a
  little soft. `PlaneMapping` composes them in one documented place and is tested against the
  transform it must reproduce.
  *Remaining:* `StarOffset`'s translation-only path is kept as a fallback rather than deleted —
...
- [x] **T-4.7** **Whole-image registration fallback**, when asterism matching fails (**new
  2026-09-07**).
...
## 8. Phase 3 — Stacking
  ...
  **Done 2026-09-09** — `registration/PhaseCorrelation.kt`, wired into `LiveRegistration` on both
  failure paths and recorded as `Method.PHASE_CORRELATION`. Pure Kotlin, including a radix-2 FFT, so
  it runs in a JVM test rather than needing the phone; OpenCV is not involved.
  *A normal session pays nothing*: it runs only where the star path has already given up.
  **What the tests had to be corrected about, twice, and it is the same lesson.** The first synthetic
  nightscape used smooth sinusoids for the landscape, and the moon won — the correlation followed the
  one moving object. So did the second, with more sinusoids. The reason is not a quirk: phase
  correlation normalises every frequency to unit magnitude, so a feature's weight is **the number of
  frequencies it occupies**, not its brightness or its area. Six sinusoids occupy six frequencies; one
  hard-edged disc occupies thousands. Only with **broadband** landscape texture — which a real
  shoreline has and a sinusoid does not — did the scene behave as the real session did and put a
  13 px moon shift at (0, 0). *That is a limitation of the method worth knowing: a frame whose only
  broadband content is the moving object will follow it.*
  **Two more things measurement moved.** The sign convention was inverted — `A · conj(B)` peaks at
  the shift taking the target back onto the reference, the negative of what every caller wants, and
  silent when wrong. And `MIN_PEAK_RATIO` was guessed at 4.0 and **would have accepted pure noise**,
  which measured 8.8; real translations measure 207–10 000, so the threshold is now 50 and the two
  populations are two orders of magnitude apart.
  *The honest limit is enforced rather than documented:* a rotating field smears the peak and the
  confidence collapses, so the fallback refuses it instead of rescuing it with the wrong model. The
  frame log says "translation only" in as many words, and the residual monitor is deliberately not
  fed — it tracks a star residual, and a frame with no stars would poison the baseline every later
  frame is judged against.
  *Acceptance on `2026-09-06_0118` still wants the phone*, but the property it tests is pinned in
  `StarlessRegistrationTest`: a moving object over a static textured scene registers at (0, 0).
...
- [~] **T-5.3** Debayer + tiled accumulator (FR-7.6): load tile T across all N frames, combine,
  ...
  **Run on a real 119-frame session 2026-09-02 (§1.38), and it is the wrong shape.** The loop reads
  a band of `tileRows + 2×margin` per tile per frame, and with 114 frames the sample budget gives an
  **8-row tile against a 160-row margin — a 41× amplification** of every read, calibration, debayer
  and warp. Measured at **~61 minutes** for one stack, of which ~16 minutes is warping pixels that
...
- [~] **T-5.4** Sigma-clipped mean as default; median / mean / kappa-sigma behind the expert
  ...
  **The profile ran 2026-09-02** (§1.38) and §1.33's estimate was low by five times: **471.7 s of
  combining for a 150-frame 12.6 MP master, single-threaded** — sixteen times the warp pass, not
...
- [ ] **T-5.7** **Validation checkpoint:** stack the same subs in Siril/DSS on the desktop and
  ...
  ~~**Needs real subs, and there are none**~~ — **there are** (§1.38). `2026-08-23_0006` has been
...
- [~] **T-6.1** Session list from a root scan + cached index (D-5): thumbnail, target label, camera,
  ...
  **The badge became the prototype's action on 2026-09-03** (§1.42): `Stack now` when a session has
  no master and `Stacked` when it has, which §6.5 said would happen "when T-5.x turns it into one".
  **The thumbnail is still a `NO STACK` placeholder** and there is now a stretched JPEG per session
  to put in it, so this is the piece that is finally unblocked.
  ...
  **Thumbnail plumbing done 2026-09-09.** `SessionSummary` now carries `previewFileName`, taken
...
- [~] **T-6.2** Sort/filter by date, target, camera, status.
  **Logic done 2026-09-09** — `session/SessionFilter.kt`. One serialisable value holding the sort
  ...
  newest-first after every trip into a detail view.
  *Two rules that are not obvious.* **Ties break by folder name, always**, or a list of sessions
...
- [~] **T-6.3** Session detail with the full frame log and manual include/exclude (FR-10.2.2).
  **The detail screen and its frame log are T-3.27.** What is left here is **manual
  ...
  stacking queue.
  **Done 2026-09-09** — `FrameRecord.included`, `SessionLog.stackable`, and `DngFrameSource` now
  reads the latter. **The override is a separate field from `accepted`, and it has to be:**
  `accepted` together with `rejectReason` is the record of *why* the gate dropped a frame, and
  overwriting it to include one would destroy that reasoning, leaving a later restack unable to get
  back to what the app actually thought.
...
- [~] **T-6.5** Restacking with versioned, non-destructive outputs and side-by-side comparison
  (FR-10.4).
  **Done 2026-09-09** — `stacking/MasterVersions.kt`, wired through `StackJob` and every reader.
  Each stack writes into `master/vN/` and records the settings that produced it, so a restack
  **cannot destroy the master someone is still deciding about** — which is the whole of FR-10.4.1,
  and the difference between a feature worth using and twenty minutes of CPU spent to find out the
  old settings were better with the old master gone.
  *Sessions stacked before this keep working*: `currentDir` falls back to `master/` itself when
...
- [~] **T-6.6** `Stale` detection when calibration masters change (FR-10.4.2). Never auto-restack.
  ...
  *A flat's version is when it was taken and how many frames went into it* — two flats from the
  same night with different counts are different masters, and a timestamp alone would call them the
  same.
  *And "nothing recorded" is `UNKNOWN`, not `FRESH`.* Every session stacked before today is in that
...
- [~] **T-6.7** Storage management: per-session and total usage, "delete subs, keep masters"
  ...
  **Logic done 2026-09-09** — `session/StoragePlan.kt`. The distinction it is built around: **a
  master is derived and the subs are a night that happened.** So "delete subs, keep the master" is
  offered *only* where a master exists — without one it would delete the whole session, and the plan
...
## 10. Phase 5 — Auto-edit
  ...
  **Logic done 2026-09-09** — `stacking/MultiNight.kt`. Mostly a set of refusals, because that is
  mostly what the feature is: a second night doubles the integration and can halve the quality, and
  the costly failure is silent — a composite quietly including a night pointed elsewhere looks
  deeper and is smeared.
  **The one hard rejection is a different camera**, and it cannot be overridden: pixel pitch, focal
  length, colour filters and the flat all change together, so the frames are not the same sky
  sampled twice, and averaging them is meaningless before any resampling (FR-11.1 says the same
  from the other end).
  *Pointing is a sanity check and not a measurement* — the fix is a compass reading taken once at
  Start — so the tolerance is 15°, azimuth is weighted by `cos(altitude)` (or a target near the
  zenith reads as wildly separated from itself), and a night with no fix is included **with a
  warning** rather than refused, since most sessions have none.
  *The reference is the deepest night*, not the first: everything is scaled onto it, so it should be
  the one with the least noise to spread.
  *Normalisation is offset **and** scale, in that order* — the offset alone leaves a brighter night
  contributing a flatter version of the same signal, the scale alone multiplies its sky along with
  its target.
...
- [~] **T-7.5** Auto-edit UI: one strength slider, before/after, re-run from the linear master,
  ...
  **Built and walked on the phone 2026-09-03** (§1.42) — the first screen in this app that shows an
  image. **Two resolutions is the design**: the master is read back decimated to ~1024 px so the
  slider keeps up, and the full frame is rendered once on Save. Decimated rather than averaged, so
  the noise the stretch is measured against is the real noise and the slider is setting what it
  appears to be. Every render restarts from the linear master, which is what makes it a slider
  rather than a ratchet. Before/after is a tap on the image, not two thumbnails.
  *Remaining:* the strength is not persisted between visits, and there is no way to re-open the
  editor from anywhere but a session's detail.
- [~] **T-7.6** MediaStore publish of the stretched JPEG (FR-9.3) + Siril/DSS-compatible export
  layout (FR-9.5).
  **Built and verified on the device 2026-09-03** (§1.42): `Pictures/StarStacker/<session>.jpg`,
  confirmed by `content query` on `MediaStore` rather than by looking at the gallery. Written
  `IS_PENDING` and cleared at the end, so a scanner cannot cache a thumbnail of a half-written file,
  and a failed write takes its own row back out. **No permission is asked for** — an app's own
  MediaStore inserts need none from API 29, and asking for the whole photo library to save one file
  is exactly what T-0.4's rule exists to prevent.
  *Remaining:* FR-9.5's export layout, and the publish happens from the stacking service rather than
  from `StackJob` — deliberately, since a diagnostic should not write to someone's photo library.
...
**Settled offline 2026-09-09: it is not a band.** The crop is a pure function of the coverage map,
so candidate shapes can be fed through the real `LinearMaster.regionFor` and checked against the two
surviving numbers — 683 291 deficient pixels and a 2804×2417 crop at (876, 476). `CoverageShapeTest`
does exactly that:

| shape holding all 683 291 deficient pixels | resulting crop |
|---|---|
| a uniform ring (48 px thick) | **3999×2975** — near the whole frame |
| uniformly scattered single pixels | slivers, far smaller than observed |
| **observed** | **2804×2417** |

**A ring is what geometry produces** — every frame is displaced, so a band around the edge is not
covered by all of them — and a ring wastes almost nothing: its crop keeps >95% of the covered area.
The observed crop keeps **57%**. So the explanation that needs no new mechanism is ruled out, and
the deficiency must have long, thin, *deep* structure.

**And the crop's own edges name where it reaches.** The first excluded column on each side is
`x = 875` and `x = 3680`; the first excluded row is `y = 475` and `y = 2893`. A deficiency touching
those four lines reproduces the observed rectangle **exactly**, at a cost of ~28 000 pixels — **4%**
of the deficient set. That is the signature to look for: not a thicker border, but a few deficient
*lines* reaching deep into the frame. **If they fall at regular intervals they are band boundaries
from the register pass**, which is the one structure in this pipeline that runs in straight lines
across a frame — and which the flat can move, because holding a 50 MB flat changes the memory
budget `registerRowsFor` divides into bands.
...
run at all. A ring's worst inset is close to its median; a spur's is many times it. It also reports the
**five worst rows and five worst columns**, which is what turns "there are lines somewhere" into
"the lines are at *these* rows" — and whether they are evenly spaced.
