...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent?.getStringExtra("diag") == "capture" && hasCameraPermission()) {
            ...
            CaptureService.start(
                ...
                // Forced, not requested. The diag harness writes into the same root as real
                // captures, and a folder of probe frames is indistinguishable from a night's work
                // once the label is arbitrary — which is how five gigabytes of irreplaceable field
                // data came to be deleted on 2026-08-19 (§1.29). Applying the prefix here rather
                // than trusting whoever types `--es label` is the whole point: it cannot be
                // forgotten in a hurry, and a folder without it is real data.
                label = DIAG_LABEL_PREFIX + (intent.getStringExtra("label") ?: "capture"),
    ...
    private companion object {
        ...
        /**
         * Every session started from the `--es diag` harness is labelled with this.
         *
         * So `2026-08-20_2337_diag-ceiling` sorts and greps apart from a real session at a glance,
         * and clearing probes never means guessing which folders were mine. **A folder without it
         * is capture data and cannot be regenerated by rebuilding anything.**
         */
        const val DIAG_LABEL_PREFIX = "diag-"
    }
}
